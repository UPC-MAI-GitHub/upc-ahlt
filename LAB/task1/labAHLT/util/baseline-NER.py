...
with open("labAHLT/resources/HSDB.txt") as h :
    for x in h.readlines() :
        external[x.strip().lower()] = "drug"

with open("labAHLT/resources/DrugBank.txt") as h :
    for x in h.readlines() :
...
suffixes = ['azole', 'idine', 'amine', 'mycin','amol','anil','anol','ol','zole','llin']
n_suffixes = ['MHD', 'PCP', 'AOM', 'CPA', 'PTX', 'TAM', 'EGF', 'TML', 'VMA','RR', 'GL']
#
# Read the Resources in order to improve the model.
# Lines to run:
# Devel: 
# python3 labAHLT/util/baseline-NER.py labAHLT/data/Devel/ result.out 
# Train: 
# python3 labAHLT/util/baseline-NER.py labAHLT/data/train/ result.out
...
def classify_token(txt):
   ...
   # drug_n
   elif txt in n_suffixes: return "drug_n"
   # groups
   else : return "NONE"
